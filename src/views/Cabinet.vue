<template>
    <div class="main-dashboard">
        <connect-wallet :showWalletOptions="showWalletOptions"
            ...
            :showProceedToMetamaskModal="showProceedToMetamaskModal" :noWallet="noWallet" />
        <TabBar />
        <SideBar />
                ...
                if (walletOpiton) {
                    ...
                    if (window.localStorage.getItem("selectedWallet") === "metamask") {
                        ...
                        if (config.SUPPORTED_BLOCKCHAINS.indexOf(Number(WC_Obj.chainId)) < 0) {
                            alert("Change your wallet extension to Avalanche Network");
                            await _this.$root.core.changeNetwork(config.PRIMARY_BLOCKCHAIN.symbol);
                        }
                        if (currentAccount) {
                            //TODO: START LOOP
