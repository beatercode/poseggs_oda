<template>
    <div
        class="your-stake-col"
        v-if="fullStakeDetails"
        :class="{
            burned: getStakingPlanData(fullStakeDetails)[6],
            'burned-claimed': getEarnedReward(fullStakeDetails) === 0 && getStakingPlanData(fullStakeDetails)[6],
        }"
    >
        <div class="your-stake-row">
            <div class="your-stake-col your-stake-nft">
                <div class="your-stake-nft-wrap">
                    <div v-if="!getStakingPlanData(fullStakeDetails)[6]" class="nft-label">{{ translatesGet("ON_STAKE") }}</div>
                    ...
                    <div class="img-wrap">
                        <div v-if="showLoader && tokenId === fullStakeDetails.event_data.tokenId" class="nft-load">
                            <div class="nft-load-icon"></div>
                        </div>
                        <img :src="getImageLink(fullStakeDetails.event_data.tokenId)" class="your-nft-img" alt="main-img" />
                    </div>
                </div>
                <div class="container-btns">
                    <button
                        v-if="fullStakeDetails.event_data.depositTypeIdx > 0 && getStakingPlanData(fullStakeDetails)[6] === false"
            ...
            <div class="your-stake-col your-stake-data">
                <div class="stake-data-header">
                    <div class="stake-id">#{{ fullStakeDetails.event_data.depositIdx + 1 }}</div>
                    <div class="stake-boosts">
                        <template v-if="fullStakeDetails.event_data.depositTypeIdx > 0 && fullStakeDetails.boostEvents.length <= 3">
                            <button
                                :disabled="getStakingPlanData(fullStakeDetails)[6] === true"
                                v-if="!isBoostApplied(1)"
                                class="st-boost"
                                @click="showMore = true"
                            >
                                <i class="icon-plus"></i>
                ...
                <div class="stake-pool-info">
                    <div class="container-pool-info">
                        <div class="stake-pool-col stake-pool">
                            ...
                            <div class="stake-pool-col-value">{{ translatesGet("POOL") }}({{ getStakingPlanData(fullStakeDetails)[4] }})</div>
                        </div>
                        <div class="stake-pool-col stake-time">
                            ...
                            <div class="stake-pool-col-value">
                                {{
                                    getStakingPlanData(fullStakeDetails)[7].toLocaleDateString("en-EN", {
                                        year: "numeric",
                                        month: "short",
                                        day: "numeric",
                                        hour: "numeric",
                        ...
                        <div class="stake-pool-col stake-time" v-if="getStakingPlanData(fullStakeDetails)[4] !== 'XL'">
                            ...
                            <div class="stake-pool-col-value">
                                {{
                                    getStakingPlanData(fullStakeDetails)[5].toLocaleDateString("en-EN", {
                                        year: "numeric",
                                        month: "short",
                                        day: "numeric",
                                        hour: "numeric",
                        ...
                        <div class="stake-pool-col stake-claimed">
                            ...
                            <div class="stake-pool-col-value">{{ getAlreadyWithdrawnReward(fullStakeDetails) }} {{ currency }}</div>
                        </div>
                        <div class="stake-pool-col stake-for-claim">
                            ...
                            <div class="stake-pool-col-value">
                                {{ getEarnedReward(fullStakeDetails) > 0 ? getEarnedReward(fullStakeDetails) : "0.00" }} {{ currency }}
                            </div>
                        </div>
                    </div>
                    <div v-if="getEarnedReward(fullStakeDetails) > 0" class="container-pool-btn">
                        <button :disabled="showLoader" @click="Claim(nft)" class="btn btn-claim">{{ translatesGet("CLAIM") }}</button>
                    </div>
                </div>
                <div class="stake-info-blocks-wrap">
                    <div class="stake-info-block">
                        <div class="stake-pool-col-name">{{ translatesGet("DAYS_OF_STAKING") }}</div>
                        <div class="stake-pool-col-value">{{ getStakingPlanData(fullStakeDetails)[0] }}</div>
                        <div class="select-boost-item-data-wrap" v-if="showMore && (selectedTimeBoost || selectedTeamBoost)">
                    ...
                    <div class="stake-info-block">
                        <div class="stake-pool-col-name">{{ translatesGet("REWARD_PERCENT_DAY") }}</div>
                        <div class="stake-pool-col-value">{{ getStakingPlanData(fullStakeDetails)[1] }}%</div>
                        <div class="select-boost-item-data-wrap" v-if="showMore && (selectedProfitBoost || selectedTeamBoost)">
                    ...
                    <div class="stake-info-block" v-if="fullStakeDetails.event_data.depositTypeIdx > 0">
                        <div class="stake-pool-col-name">{{ translatesGet("TOTAL_ST_PERCENT") }}</div>
                        <div class="stake-pool-col-value">
                            {{ getStakingPlanData(fullStakeDetails)[2] > 0 ? getStakingPlanData(fullStakeDetails)[2] + "%" : "Days staked * daily percent" }}
                        </div>
                        <div class="select-boost-item-data-wrap" v-if="showMore && (selectedProfitBoost || selectedTeamBoost || selectedTimeBoost)">
                    ...
                    <div class="stake-info-block" v-if="fullStakeDetails.event_data.depositTypeIdx > 0">
                        <div class="stake-pool-col-name">{{ translatesGet("EXPECTED_REWARD") }}</div>
                        <div class="stake-pool-col-value">
                            {{
        ...
        <div class="your-stake-row your-stake-more" :class="{ active: showMore }">
            <div class="your-stake-boosts">
                <div v-if="isTimeBoostApplied" class="your-stake-boost-wrap your-stake-boost-wrap-inactive">
                    ...
                    <div class="stake-boost-describe">{{ translatesGet("BOOSTER_INACTIVE") }}</div>
                </div>
                <template v-else>
                    <div class="your-stake-boost-wrap" v-if="!timeBoosts || timeBoosts.length === 0">
                        <div class="stake-boost-header">
                            <div class="stake-boost-header-icon boost-time"></div>
                            <div class="title">{{ translatesGet("BOOSTER_TIME") }}</div>
                            <router-link :to="{ name: 'Boosts' }" class="btn">{{ translatesGet("BUY") }}</router-link>
                        </div>
                        <div class="stake-boost-describe">{{ translatesGet("NO_TIME_BOOST") }}</div>
                    </div>
                    <div class="your-stake-boost-wrap" v-else>
                        <div
                            class="select-boost"
                            :class="{ 'select-boost-active': selectList1 }"
                            @mouseover="selectList1 = true"
                            @mouseout="selectList1 = false"
                        >
                            ...
                            <ul class="list-select-boost">
                                <li class="select-boost-item" v-for="boost of timeBoosts" :key="boost.tokenId" @click="selectedTimeBoost = boost">
                                    <div class="select-boost-img">
                        ...
                        <button :disabled="showLoader" class="btn btn-submit-select" @click="activateBoost('time')">{{ translatesGet("ACTIVE") }}</button>
                    </div>
                </template>
                ...
                <template v-else>
                    <div class="your-stake-boost-wrap" v-if="!profitBoosts || profitBoosts.length === 0">
                        ...
                        <div class="stake-boost-describe">{{ translatesGet("NO_PROFIT_BOOST") }}</div>
                    </div>
                    <div class="your-stake-boost-wrap" v-else>
                        <div
                            class="select-boost"
                            :class="{ 'select-boost-active': selectList2 }"
                            @mouseover="selectList2 = true"
                            @mouseout="selectList2 = false"
                        >
                            <div class="selected" v-if="showMore">
                                <div class="select-boost-item">
                                    <div class="select-boost-img">
                            ...
                            <ul class="list-select-boost">
                                <li class="select-boost-item" v-for="boost of profitBoosts" :key="boost.tokenId" @click="selectedProfitBoost = boost">
                                    <div class="select-boost-img">
                                        <img :src="boost.image" alt="" />
                                    </div>
                                    <div class="select-boost-data">
                        ...
                        <button :disabled="showLoader" class="btn btn-submit-select" @click="activateBoost('profit')">{{ translatesGet("ACTIVE") }}</button>
                    </div>
                </template>
                ...
                <template v-else>
                    <div class="your-stake-boost-wrap" v-if="!teamBoosts || teamBoosts.length === 0">
                        <div class="stake-boost-header">
                            <div class="stake-boost-header-icon boost-team"></div>
                            <div class="title">{{ translatesGet("BOOSTER_TEAM") }}</div>
                            <router-link :to="{ name: 'Program' }" class="btn">{{ translatesGet("LEAD_PROG") }}</router-link>
                        </div>
                        <div class="stake-boost-describe">{{ translatesGet("NO_TEAM_BOOST") }}</div>
                    </div>
                    <div class="your-stake-boost-wrap" v-else>
                        <div
                            class="select-boost"
                            :class="{ 'select-boost-active': selectList3 }"
                            @mouseover="selectList3 = true"
                            @mouseout="selectList3 = false"
                        >
                            <div class="selected" v-if="showMore">
                                <div class="select-boost-item">
                                    <div class="select-boost-img">
                            ...
                            <ul class="list-select-boost">
                                <li class="select-boost-item" v-for="boost of teamBoosts" :key="boost.tokenId" @click="selectedTeamBoost = boost">
                                    <div class="select-boost-img">
                                        <img :src="boost.image" alt="" />
                                    </div>
                                    <div class="select-boost-data">
                        ...
                        <button :disabled="showLoader" class="btn btn-submit-select" @click="activateBoost('team')">{{ translatesGet("ACTIVE") }}</button>
                    </div>
                </template>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            async activateBoost(type) {
                try {
                    const boostToApply = type === "profit" ? this.selectedProfitBoost : type === "time" ? this.selectedTimeBoost : this.selectedTeamBoost;
                    if (this.boostsApplied.find((el) => el.metadata.type.toLowerCase() === type.toLowerCase())) {
                    ...
                    res = await this.$root.core.addBoost(this.nft.event_data.depositIdx, boostToApply.tokenId);
                    this.$store.commit("push_notification", {
                        type: "warning",
                        typeClass: "warning",
            ...
            getBoostImage(nft) {
                return this.$root.core.getBoostMetadata(nft.event_data.boostTokenId).then((res) => res.image);
            },
            getStakingPlanData(nft) {
                ...
                if (stakeType > 0) {
                    ...
                    expectedReward =
                        ((((Number(nft.event_data.amount) * (end - start)) / 3600) * dailyPerc) / 24 / 100).toFixed(4) > 0
                            ? ((((Number(nft.event_data.amount) * (end - start)) / 3600) * dailyPerc) / 24 / 100).toFixed(4)
                            : "0.0000";
                } else {
                ...
                return [period, dailyPerc.toFixed(2), totalProfit, expectedReward, size, end, isStakeExpired, new Date(nft.timestamp * 1000)];
            },
            ...
            getEarnedReward(stake) {
                ...
                let res = (Number(amount) * ((dailyPerc / 24) * hoursPassedSinceStart)) / 100;
            ...
            getImageForBoost(type) {
                return this.fullStakeDetails.boostEvents.find((el) => el.metadata.type === type)?.metadata.image;
            },
        },
        computed: {
            ...mapState(["userStakes", "currentBlockchain", "currentAddress", "userNftsData", "currency"]),
            ...
            timeBoosts() {
                if (
                    ...
                    const arr =
                        this.userNftsData[this.currentBlockchain][conf[this.currentBlockchain].BOOST_NFT_CONTRACT].filter((el) => el.type === "TIME") || [];
                    arr.sort((a, b) => Number(b.attributes[1].value.replace("%", "")) - Number(a.attributes[1].value.replace("%", "")));
                    return arr;
                }
                return null;
            },
            teamBoosts() {
                if (
                    ...
                    const arr =
                        this.userNftsData[this.currentBlockchain][conf[this.currentBlockchain].BOOST_NFT_CONTRACT].filter((el) => el.type === "TEAM") || [];
            ...
            profitBoosts() {
                if (
                    ...
                    const arr =
                        this.userNftsData[this.currentBlockchain][conf[this.currentBlockchain].BOOST_NFT_CONTRACT].filter((el) => el.type === "PROFIT") || [];

                    arr.sort((a, b) => Number(b.attributes[2].value.replace("%", "")) - Number(a.attributes[2].value.replace("%", "")));
                    return arr;
                }
                return null;
            },
            isProfitBoostApplied() {
                return Boolean(this.boostsApplied.find((el) => el.metadata.type === "PROFIT"));
            },
            isTimeBoostApplied() {
                return Boolean(this.boostsApplied.find((el) => el.metadata.type === "TIME"));
            },
            isTeamBoostApplied() {
                return Boolean(this.boostsApplied.find((el) => el.metadata.type === "TEAM"));
            },
        ...
        watch: {
                ...
                if (newVal) {
                    ...
                    if (!this.isTeamBoostApplied) {
                        ...
                        if (this.selectedTeamBoost) {
                            increasedDays = increasedDays + (increasedDays * Number(this.teamBoosts[0].attributes[1].value.replace("%", ""))) / 100;
                            increasedPercent = increasedPercent + (increasedPercent * Number(this.teamBoosts[0].attributes[2].value.replace("%", ""))) / 100;
                        }
                    }
                    if (!this.isProfitBoostApplied) {
                        this.selectedProfitBoost = this.profitBoosts[0] || null;
                        if (this.selectedProfitBoost) {
                            increasedPercent = increasedPercent + (increasedPercent * Number(this.profitBoosts[0].attributes[2].value.replace("%", ""))) / 100;
                        }
                    }
                    if (!this.isTimeBoostApplied) {
                        this.selectedTimeBoost = this.timeBoosts[0] || null;
                        if (this.selectedTimeBoost) {
                            increasedDays = increasedDays + (increasedDays * Number(this.timeBoosts[0].attributes[1].value.replace("%", ""))) / 100;
                        }
                    }
                    this.increasedPercent = (increasedPercent * increasedDays).toFixed(2);
                    this.increasedProfit = ((Number(this.fullStakeDetails.event_data.amount) * this.increasedPercent) / 100).toFixed(4);
                } else {
                    this.selectedProfitBoost = null;
                    this.selectedTimeBoost = null;
                    this.selectedTeamBoost = null;
            ...
            boostsApplied: function (newVal, oldVal) {
                if (newVal && oldVal && newVal.length !== oldVal.length && this.isBoostApplying && !this.isUnstaking) {
                    this.showLoader = false;
                    this.showMore = false;
                    this.isBoostApplying = false;
            ...
            selectedProfitBoost: function (newVal) {
                if (newVal) {
                    ...
                    let increasedDays = Number(days.replace("days", ""));
                    let increasedPercent = Number(daily);
                    increasedPercent = increasedPercent + (increasedPercent * Number(newVal.attributes[2].value.replace("%", ""))) / 100;
                    increasedDays = increasedDays + increasedDays * (Number(this.selectedTimeBoost?.attributes[1].value.replace("%", "")) / 100 || 0);
                    this.increasedPercent = (increasedPercent * increasedDays).toFixed(2);
                    this.increasedProfit = ((Number(this.fullStakeDetails.event_data.amount) * this.increasedPercent) / 100).toFixed(4);
                }
            },
            selectedTeamBoost: function (newVal) {
                if (newVal) {
                    ...
                    let increasedPercent = Number(daily);
                    increasedPercent = increasedPercent + (increasedPercent * Number(newVal.attributes[2].value.replace("%", ""))) / 100;
                    increasedDays = increasedDays + increasedDays * (Number(newVal.attributes[1].value.replace("%", "").replace("-", "")) / 100 || 0);
            ...
            selectedTimeBoost: function (newVal) {
                if (newVal) {
                    ...
                    let increasedPercent = Number(daily);
                    increasedPercent =
                        increasedPercent + increasedPercent * (Number(this.selectedProfitBoost?.attributes[2].value.replace("%", "")) / 100 || 0);
                    increasedDays = increasedDays + (increasedDays * Number(newVal.attributes[1].value.replace("%", ""))) / 100;
                    this.increasedPercent = (increasedPercent * increasedDays).toFixed(2);
                    this.increasedProfit = ((Number(this.fullStakeDetails.event_data.amount) * this.increasedPercent) / 100).toFixed(4);
                }
            },
            getLastWithdrawalTime: function (newVal, oldVal) {
                if (newVal > oldVal && oldVal !== null && this.showLoader === true && !this.isUnstaking) {
                    this.showLoader = false;
                    this.tokenId = "";
                    this.$store.commit("clear_notifications", []);
                    this.$store.commit("push_notification", {
