...
export default class Core {
    ...
    async init() {
        if (window.localStorage.getItem("selectedWallet") === "walletconnect") {
            ...
            for (let chainId of conf.SUPPORTED_BLOCKCHAINS) {
                ...
                this[`alphaTribeNft_${chainId}`] = new ethers.Contract(conf[chainId].NFT_CONTRACT, nftAbi, this[`provider_${chainId}`]).connect(this[`provider_${chainId}`]);
                ...
                this[`USDC_${chainId}`] = new ethers.Contract(conf[chainId].ERC20_CONTRACT, erc20, this[`provider_${chainId}`]).connect(this[`provider_${chainId}`]);
                ...
                if (blockchain === Number(chainId)) {
                    ...
                    //TODO initialize ERC20?contract
                    this[`alphaTribeNft_${chainId}`] = new ethers.Contract(conf[chainId].NFT_CONTRACT, nftAbi, this.provider).connect(this.signer);
            ...
            for (let chainId of conf.SUPPORTED_BLOCKCHAINS) {
                ...
                this[`alphaTribeNft_${chainId}`] = new ethers.Contract(conf[chainId].NFT_CONTRACT, nftAbi, this[`provider_${chainId}`]).connect(
                ...
                this[`USDC_${chainId}`] = new ethers.Contract(conf[chainId].ERC20_CONTRACT, erc20, this[`provider_${chainId}`]).connect(
                    this[`provider_${chainId}`]
                );
                ...
                */
                ...
                this[`USDC_${chainId}`] = new ethers.Contract(conf[chainId].ERC20_CONTRACT, erc20, this[`provider_${chainId}`]);
                ...
                if (blockchain === Number(chainId)) {
                    ...
                    // TODO  Create ERC20? contract instance
                    if (this[`alphaTribeNft_${chainId}`]) this[`alphaTribeNft_${chainId}`].connect(this[`provider_${chainId}`]);
                    ...
                    if (this[`USDC_${chainId}`]) this[`USDC_${chainId}`].connect(this[`provider_${chainId}`]);
    ...
    async buyNFT(buyer, refs, indexPlan) {
        const res = await this[`alphaTribeNft_${this.currentBlockchain}`].mintNft(buyer, refs, indexPlan);
        return res;
    }

    async buyLOOTBOX(refs, indexPlan) {
        const res = await this[`alphaTribeNft_${this.currentBlockchain}`].mintLootbox(refs, indexPlan);
        return res;
    }
    ...
    //     const rawTransaction = await this[`USDC_${this.currentBlockchain}`].approve(this[`boostNft_${this.currentBlockchain}`].address, amount);
    //     return rawTransaction;
    // }

    async claimLeadership() {
        const res = await this[`alphaTribeNft_${this.currentBlockchain}`].claimLeadership();
        return res;
    }

    async claimLeaderBoost() {
        return await this[`alphaTribeNft_${this.currentBlockchain}`].claimBoost();
    }
    ...
    async transferNft(type, from, id, recipient) {
        if (type === "boost") {
            ...
            const res = await this[`alphaTribeNft_${this.currentBlockchain}`].transferFrom(from, recipient, id);
            return res;
        }
    }
    ...
    async getUserNftsInChain(address, chainId) {
        ...
        for (let ni of buyersNftIds) {
            ...
            eggInfo.collection = "AlphaTribes-NFT";
            eggInfo.description = "";
    ...
    async getUserNftsBoostsInChain(address, chainId) {
        ...
        for (let ni of buyersNftIds) {
            ...
            boostFinal.collection = "AlphaTribes-BOOST";
    ...
    async getLeaderStats(address) {
        try {
            let leader = null;
            try {
                leader = await this[`alphaTribeNft_${this.currentBlockchain}`].findLeader(address);
            ...
            if (leader.toLowerCase() === conf[this.currentBlockchain].DEFAULT_REFERRER.toLowerCase()) {
                ...
                const res = await this[`alphaTribeNft_${this.currentBlockchain}`].getBuyerReferralsStats(leader);
                const latest_level = (await this.getLastClaimedLevelByLeader(address)) || 0;
                let referralsNumber = 0;
                for (let i = 0; i < 5; i++) {
                    referralsNumber += Number(res[3][i].toString());
    ...
    async getUserBalance(address) {
        try {
            ...
            if (this.currentBlockchain === 43114 || this.currentBlockchain === 43113) {
                tokenBalance = await this[`USDC_${this.currentBlockchain}`].balanceOf(address);
                return {
    ...
    async isApprovedForAll(userAddress, type) {
        if (type === "ALPHAEGG") {
            return await this[`alphaTribeNft_${this.currentBlockchain}`].isApprovedForAll(userAddress, this[`stake_${this.currentBlockchain}`].address);
        } else if (type === "BOOST") {
    ...
    async approve(nftToken, userAddress) {
        if (nftToken === "ALPHAEGG") {
            ...
            // await this[`alphaTribeNft_${this.currentBlockchain}`].getApproved(id);
            // console.log(res);
            if (!res) {
                res = await this[`alphaTribeNft_${this.currentBlockchain}`].setApprovalForAll(this[`stake_${this.currentBlockchain}`].address, "true");
    ...
    async estimateGas(method, params) {
        if (method === "buy") {
            ...
            return await this[`alphaTribeNft_${this.currentBlockchain}`].estimateGas[method](params.ref, {
                value: amount._hex
            });
